...
class Order extends Component {
  ...
  static async getInitialProps({ query, req, res }) {
    ...
    const orders = jsonValidation(query.selected) ? JSON.parse(query.selected) : [];
  ...
  getTotalPrice = () => {
    ...
    return orders.reduce((price, order) => price + order.p_price * order.c_amount, 0);
  };
  ...
  postOrders = async () => {
    ...
    if(res.success) {
      Router.push(URL.MYPAGE.ORDERS.link)
    } else {
      Router.push(URL.CART.link)
    }
  };
  ...
  render() {
    ...
    return (
      <div>
        ...
        >
          <Descriptions.Item label="총금액" span={1}>{total}</Descriptions.Item>
          ...
          <Descriptions.Item label="이메일" span={3}>{user.u_email}</Descriptions.Item>
          <Descriptions.Item label="주소" span={3}>{user.u_address}</Descriptions.Item>
        </Descriptions>
        )}
